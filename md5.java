...
public class md5 extends JFrame {
	public static void main(String[] args) {
		String mess = "The package of Enchor is still open. Please pack it again and be cafeful because of fragile products.";
		MD5(mess);
	}
	...
	public static String MD5(String mess) {
		...
		for (int i=0; i<MessBinary.length; i++) {
			...
			for (int j=0; j< 64; j++) {
				...
				F = (int) (F + A + k[i] + M[G]); // M[g] must be a 32-bits block
		        A = D;
		        D = C;
		        C = B;
	...
	public static int[] plitM(StringBuilder messBinary) {
		...
		for (int i=0; i<16; i++) {
			...
			m[i] = Integer.parseInt(binary32,2);
		}
		return m;
	}
	...
	public static byte[] Padding(byte[] b) {
		byte[] newb;
		if (b.length%64 > 55) 
			newb = new byte[(int) Math.ceil(b.length/64)*64];
